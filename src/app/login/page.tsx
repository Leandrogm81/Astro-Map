...
import { Moon, Star, KeyRound, Lock, User, Eye, EyeOff, UserPlus, Sparkles, Loader2 } from 'lucide-react';
  ...
  const handleGuestLogin = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setError(data.error || 'Erro ao acessar como visitante');
      }
    } catch (err) {
      setError('Erro de conexão ao tentar acessar como visitante');
    } finally {
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-xs uppercase tracking-widest">
                <span className="px-3 bg-slate-900 text-slate-500 font-bold">ou</span>
              </div>
            </div>
            ...
            <button
              ...
              disabled={loading || guestLoading}
              className="w-full flex items-center justify-center gap-2 py-3 px-4 border border-white/10 rounded-xl shadow-lg text-sm font-bold uppercase tracking-wider text-white bg-white/5 hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
              {guestLoading ? (
            ...
            <p className="text-[10px] text-center text-slate-500 mt-4 leading-relaxed">
              O acesso de visitante permite gerar <span className="text-gold-500/80">um relatório de IA</span> e expira em 3 dias.
            </p>
          </form>
        </div>
      </div>
